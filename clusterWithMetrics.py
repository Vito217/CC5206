...
def clusters_frecuent_terms(x, vectorizer, its, n_clusters, cluster_type="kmeans", terms=True):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument("-show_terms", type=str,required=False)
    ...
    plotting = False if pargs.plotting == 'False' else True
    showterms = False if pargs.show_terms == 'False' else  True
    ...
    save = pargs.save
    sse = []
    list_k =range(get_number_of_clusters(labels),n_clusters+1)
    for k in list_k:
        for method in cluster_type:
            inertia = clusters_frecuent_terms(x, vectorizer, its, k, method, showterms)
            sse.append(inertia)
            if plotting:
